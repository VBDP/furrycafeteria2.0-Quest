...
namespace Cyan.CT.Editor
{
    public class CyanTriggerCustomNodeInspectorManager
    {
        ...
        private static readonly List<ICyanTriggerCustomNodeInspector> ExternalInspectors =
            new List<ICyanTriggerCustomNodeInspector>();
        ...
        public static void AddExternalCustomInspector(ICyanTriggerCustomNodeInspector inspector)
        {
            if (_instance != null)
            {
                _instance.AddCustomInspector(inspector);
            }
            else
            {
                ExternalInspectors.Add(inspector);
            }
        }
        ...
        private CyanTriggerCustomNodeInspectorManager()
        {
            ...
            ICyanTriggerCustomNodeInspector[] inspectors =
            {
                ...
                new CyanTriggerCustomNodeInspectorAnimatorGetParameter(typeof(bool)),
                new CyanTriggerCustomNodeInspectorAnimatorGetParameter(typeof(int)),
                new CyanTriggerCustomNodeInspectorAnimatorGetParameter(typeof(float)),
            };

            foreach (var inspector in inspectors)
            {
                AddCustomInspector(inspector);
            }

            foreach (var externalInspector in ExternalInspectors)
            {
                AddCustomInspector(externalInspector);
            }
            ExternalInspectors.Clear();
        }

        private void AddCustomInspector(ICyanTriggerCustomNodeInspector inspector)
        {
