...
namespace Cyan.CT.Editor
{
    public static class CyanTriggerSerializedPropertyUtils
    {
            ...
            string comment = null)
        {
            ...
            if (data == null)
            {
                CyanTriggerSerializableObject.UpdateSerializedProperty(dataProperty, CyanTriggerPropertyEditor.GetDefaultForType(type));
            }
            else
            {
            ...
            CyanTriggerActionVariableDefinition[] variableDefinitions)
        {
            ...
            for (int cur = 0; cur < variableDefinitions.Length; ++cur)
            {
                ...
                {
                    data = variableDef.defaultValue?.Obj ?? CyanTriggerPropertyEditor.GetDefaultForType(type);
                }
