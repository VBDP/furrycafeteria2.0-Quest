...
namespace Cyan.CT.Editor
{
    public static class CyanTriggerEditorUtils
    {
        ...
        private const string VrcGroupURL = "https://vrc.group/CYNTRG.2578";
        ...
        private static void DrawLinks(bool showSettingsButton)
        {
            ...
            if (GUI.Button(buttonRect, new GUIContent("Wiki", WikiURL)))
            {
                Application.OpenURL(WikiURL);
            }
            UpdateButton();

            if (GUI.Button(buttonRect, new GUIContent("Discord", DiscordURL)))
            {
                Application.OpenURL(DiscordURL);
            }
            UpdateButton();

            if (GUI.Button(buttonRect, new GUIContent("Patreon", PatreonURL)))
            {
                Application.OpenURL(PatreonURL);
            }
            UpdateButton();
            
            if (GUI.Button(buttonRect, new GUIContent("VRC Group", VrcGroupURL)))
            {
                Application.OpenURL(VrcGroupURL);
            }
            UpdateButton();
        ...
        {
            string tooltip = $"View Custom Action:\n{udonActionGroup.GetNamespace()}";
