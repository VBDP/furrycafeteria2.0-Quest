...
namespace Cyan.CT.Editor
{
    public class CyanTriggerSerializableInstanceEditor
    {
        ...
        public void Dispose()
        {
            ...
            // On closing any editor, ensure that the dynamic type editors are cleared to prevent large memory leaks.
            // Creating actions will always add to the expand lists for each type, but deleting actions does not remove.
            // The only safe time to remove is on editor disposed, but this means that if multiple editors are open,
            // all will be cleared. Not the best experience, but an edge case that will be ignored. 
            CyanTriggerTypePropertyEditorManager.ClearDynamicTypeExpandValues();
        }
        ...
        private void ResetValues()
        {
            ...
            for (int i = 0; i < _eventListSize; ++i)
            {
                ...
                {
                    eventData = _eventInstanceRenderData[i] = new CyanTriggerActionInstanceRenderData
                    {
                        Property = _eventsProperty.GetArrayElementAtIndex(i)
                            .FindPropertyRelative(nameof(CyanTriggerEvent.eventInstance)),
                    ...
                    // TODO, do this better?
                    _eventOptionRenderData[i] = new CyanTriggerActionInstanceRenderData
                    {
                ...
                if (eventData.NeedsReinitialization)
                {
        ...
        private void UpdateOrCreateActionTreeForEvent(int index)
        {
            ...
            void RequestCustomActionCreation(List<int> selectedActions)
            {
                CyanTriggerCustomActionGetNameWindow.RequestCustomActionNamespace(
                    _cyanTriggerDataInstance, 
                    _baseEditor.GetTarget(),
                    index,
                    selectedActions);
            }
            ...
            if (_eventActionTrees[index] == null)
            {
                ...
                _eventActionTrees[index].RequestCustomActionCreation = RequestCustomActionCreation;
            }
            else
            {
                ...
                actionTree.RequestCustomActionCreation = RequestCustomActionCreation;
            }
        }
            ...
            CyanTriggerActionInfoHolder actionInfo)
        {
            ...
            if (actionInfo.IsAction())
            {
                ...
                float width = (customActionRect.width - EditorGUIUtility.labelWidth - spaceBetween - 2 - docIconSize) / 2;;
                Rect labelRect = new Rect(customActionRect.x, customActionRect.y, EditorGUIUtility.labelWidth, customActionRect.height);
                Rect customActionDocRect = new Rect(customActionRect.xMax - docIconSize, customActionRect.y + (customActionRect.height - docIconSize) * 0.5f, docIconSize, docIconSize);
                ...
                string infoTooltip = "This event is a Custom Action. This means it was made through the combination of other actions. You can view the actions by clicking the \"View Actions\" button. You can view the definition describing the Custom Action by clicking the \"View Definition\" button. Click the ? button to view the wiki for Custom Actions"; 
                EditorGUI.LabelField(labelRect, new GUIContent("Custom Action Info*", infoTooltip));
                CyanTriggerEditorUtils.DrawDocumentationButton(customActionDocRect, "Custom Actions", CyanTriggerDocumentationLinks.CustomAction);
                ...
                if (actionDefinition is CyanTriggerActionGroupDefinitionUdonAsset udonCustomAction)
                {
                    ...
                    {
                        Selection.objects = new UnityEngine.Object[] { customProgram };
                    }
                }
                
                if (GUI.Button(definitionRect, new GUIContent("View Definition", "View the Custom Action Definition.")))
                {
                    Selection.objects = new UnityEngine.Object[] { actionDefinition };
                }
        ...
        private void ShowEventOptionsMenu(int eventIndex)
        {
            ...
            GUIContent createCustomAssetGuiContent = new GUIContent("Create Custom Action from Event actions");
            ...
            bool anyInstanceCustomActions = 
                _eventActionTrees[eventIndex] != null && _eventActionTrees[eventIndex].ContainsAnyInstanceCustomActions();
            
            if (anyInstanceCustomActions || 
                (_baseEditor.IsSceneTrigger() && !CyanTriggerProgramAssetExporter.CanExportAsset(_cyanTriggerSerializableInstance?.udonBehaviour)))
            {
        ...
        public void GetUsedVariablesForEventActions(
            int eventIndex, 
            List<int> actions,
            bool updateUsage,
            ...
            out List<CyanTriggerEditorVariableOption> missingVariables)
        {
            ResetValues();
            UpdateVariableScope();
            var actionTree = _eventActionTrees[eventIndex];
            actionTree.GetUsedVariables(actions, out inputVariables, out outputVariables, out missingVariables);
            
            if (updateUsage)
            {
                // If variable guid listed here is created in the action list, remove creation and only use the guid.
                HashSet<string> allGuids = new HashSet<string>();
                foreach (var variable in inputVariables) allGuids.Add(variable.ID);
                foreach (var variable in outputVariables) allGuids.Add(variable.ID);
                foreach (var variable in missingVariables) allGuids.Add(variable.ID);
                actionTree.ForceVariableOutputsToNotCreateNew(allGuids);
    ...
    public class CyanTriggerEditorVariableOption
    {
        ...
        // Hacked in here for experimental CodeBlocks to allow for searching for variables by display name rather than internal names
        public string DisplayName;
        
        public static CyanTriggerEditorVariableOption Clone(CyanTriggerEditorVariableOption option)
        {
