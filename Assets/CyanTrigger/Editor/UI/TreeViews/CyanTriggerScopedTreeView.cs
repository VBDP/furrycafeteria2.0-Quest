...
namespace Cyan.CT.Editor
{
    ...
    public abstract class CyanTriggerScopedTreeView : TreeView
    {
        ...
        private Action<Rect, CyanTriggerScopedTreeItem> _fixDragInsertProperties;
        ...
        private void InitializeReflectionItems()
        {
            ...
            Func<int> getRowMarkerControlID = () => 0;
            Func<float> getDraggingInsertionMarkerYPosition = () => 0f;
            Action<float> setDraggingInsertionMarkerYPosition = _ => { };
#endif
            ...
            if (draggingInfo != null)
            {
                ...
                MethodInfo getDropTargetControlIdMethodInfo = dragging.GetType().GetMethod("GetDropTargetControlID");
                // ReSharper disable once PossibleNullReferenceException
                getDropTargetControlID = () => (int) getDropTargetControlIdMethodInfo.Invoke(dragging, Array.Empty<object>());
                
#if UNITY_2022_3_OR_NEWER
                MethodInfo getRowMarkerControlIdMethodInfo = dragging.GetType().GetMethod("GetRowMarkerControlID");
                // ReSharper disable once PossibleNullReferenceException
                getRowMarkerControlID = () => (int) getRowMarkerControlIdMethodInfo.Invoke(dragging, Array.Empty<object>());
                
                
                // DropData is created new multiple times.
                // These methods need to retrieve it for every new drag otherwise data is stale.
                getDraggingInsertionMarkerYPosition = () =>
                    {
                        FieldInfo dropDataField = dragging.GetType().GetField("m_DropData", BindingFlags.NonPublic | BindingFlags.Instance);
                        // ReSharper disable once PossibleNullReferenceException
                        var dropData = dropDataField.GetValue(dragging);
                        var insertionMarkerYPositionFieldInfo = dropData.GetType().GetField("insertionMarkerYPosition");
                        return (float)insertionMarkerYPositionFieldInfo.GetValue(dropData);
                    };
                setDraggingInsertionMarkerYPosition = (value) =>
                    {
                        FieldInfo dropDataField = dragging.GetType().GetField("m_DropData", BindingFlags.NonPublic | BindingFlags.Instance);
                        // ReSharper disable once PossibleNullReferenceException
                        var dropData = dropDataField.GetValue(dragging);
                        var insertionMarkerYPositionFieldInfo = dropData.GetType().GetField("insertionMarkerYPosition");
                        insertionMarkerYPositionFieldInfo.SetValue(dropData, value);
                    };
#endif
            }
            ...
            if (guiInfo != null)
            {
                ...
                _fixDragInsertProperties = (rect, item) =>
                {
                    int id = getItemControlID(item);
                    
                    if (id == getDropTargetControlID())
                    {
...
#if UNITY_2022_3_OR_NEWER
                    // Special check only for first item due to it having rect y of 0 and insertionMarkerYPosition of 0,
                    // preventing drawing of the insertion line at the top of the table...
                    if (id == getRowMarkerControlID()
                        && item.Index == 0
                        && getDraggingInsertionMarkerYPosition() < rect.yMax)
                    {
                        // Set insertionMarkerYPosition to any value greater than 0 just to force the insertion line to be drawn.
                        setDraggingInsertionMarkerYPosition(1f);
                    }
#endif
                };
        ...
        protected void KeepOnlyParents(List<int> selected)
        {
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            {
                _fixDragInsertProperties(args.rowRect, (CyanTriggerScopedTreeItem)args.item);
            }
        ...
        // Assumes sorted.
        protected List<int> GetAllIdsIncludingChildren(List<int> items, bool asIndices = false)
        {
            ...
            foreach (int id in items)
            {
                ...
                {
                    finalIds.Add(asIndices? i : i + IdStartIndex);
                    indHash.Add(i);
                }
            }
        ...
        {
            List<int> nextSelected = new List<int>();
            HashSet<int> selection = new HashSet<int>(GetSelection());
            for (int i = 0; i < Size; ++i)
            {
                int id = i + IdStartIndex;
                if (!selection.Contains(id) && Items[i] != null && CanItemBeMoved(Items[i]))
                {
                    nextSelected.Add(id);
                }
            }
        ...
        public void SelectIndices(List<int> selection)
        {
            for (int i = 0; i < selection.Count; ++i)
            {
                selection[i] += IdStartIndex;
            }
