...
namespace Cyan.CT.Editor
{
    public abstract class CyanTriggerVariableTreeView : CyanTriggerScopedDataTreeView<CyanTriggerVariableTreeView.VariableExpandData>
    {
        ...
        protected abstract string GetVariableIDForIndex(int index);
        ...
        protected override float GetCustomRowHeight(int row, TreeViewItem item)
        {
            ...
            object data = null;
            string variableName = GetNameForIndex(scopedItem.Index);
            SerializedProperty dataProperty = GetDataPropertyForIndex(scopedItem.Index);
            bool updatePlaymodeVariables = ShouldDrawEditorInPlaymode(variableName);
            if (updatePlaymodeVariables)
            {
                bool valuesDiffer = false;
                data = GetUdonValueForVariable(variableName, ref valuesDiffer);
            }
            else
            {
            ...
            if (dirty)
            {
                ...
                if (updatePlaymodeVariables)
                {
                    SetUdonValueForVariable(variableName, data);   
                }
                else
                {
            ...
            string varId = GetVariableIDForIndex(scopedItem.Index);
            height = height + SpaceBetweenRowEditor * 2 + 
                   CyanTriggerPropertyEditor.HeightForEditor(type, data, true, ref expandData.List, DelayRefreshRowHeight, varId);
        ...
        private void DrawSingleLineVariableEditor(Rect cellRect, CyanTriggerScopedTreeItem item, Type type)
        {
            ...
            else
            {
                ...
                string varId = GetVariableIDForIndex(item.Index);
                
                if (ShouldDrawEditorInPlaymode(varName))
                {
                    DrawEditorInPlaymode(varName, type, cellRect, varId);
                }
                else
                {   
                    ...
                    CyanTriggerPropertyEditor.DrawEditor(dataProperty, cellRect, GUIContent.none, type, ref heightChanged, varId, false);
                    if (heightChanged)
                    {
                        DelayRefreshRowHeight();
                    }
        ...
        private void DrawMultilineVariableEditor(int index, Rect rect)
        {
            ...
            string varId = GetVariableIDForIndex(index);
            if (!CyanTriggerPropertyEditor.TypeHasSingleLineEditor(type) && expandData.IsExpanded)
            {
                ...
                if (type.IsArray)
                {
                    ...
                    {
                        DrawArrayEditorInPlaymode(variableName, type, rect, new GUIContent(display), ref expandData.IsExpanded, ref expandData.List, varId);
                    }
                    else
                    {
                ...
                else
                {
                    if (ShouldDrawEditorInPlaymode(variableName))
                    {
                        DrawEditorInPlaymode(variableName, type, rect, varId);
                    }
                    else
                    {
                        EditorGUI.BeginDisabledGroup(IsPlaying);
                        ...
                        bool heightChanged = false;
                        CyanTriggerPropertyEditor.DrawEditor(dataProperty, rect, GUIContent.none, type, 
                            ref heightChanged, varId,false);
        ...
        private void DrawEditorInPlaymode(string variableName, Type type, Rect rect, string varId)
        {
            bool valuesDiffer = false;
            ...
            bool dirty = false;
            bool heightChanged = false;
            obj = CyanTriggerPropertyEditor.DisplayPropertyEditor(rect, GUIContent.none, type, obj, ref dirty, ref heightChanged, varId, false);
        ...
        private void DrawArrayEditorInPlaymode(string variableName, Type type, Rect rect, GUIContent displayText, ref bool arrayExpand, ref ReorderableList list, string varId)
        {
            bool valuesDiffer = false;
            object obj = GetUdonValueForVariable(variableName, ref valuesDiffer);
            ...
            bool dirty = false;
            obj = CyanTriggerPropertyEditor.DisplayArrayPropertyEditor(displayText, type, obj, ref dirty, ref arrayExpand, DelayRefreshRowHeight, ref list, varId, false, rect);
