...
namespace Cyan.CT.Editor
{
    public class CyanTriggerActionInstanceRenderData
    {
        ...
        public void ClearCache()
        {
            _cachedComment = null;
            _cachedExpanded = -1;
            ControlHint = Guid.NewGuid().ToString();
        }
    ...
    public class CyanTriggerActionTreeView : CyanTriggerScopedDataTreeView<CyanTriggerActionInstanceRenderData>
    {
        ...
        public Action<List<int>> RequestCustomActionCreation;
        ...
        public void ForceVariableOutputsToNotCreateNew(HashSet<string> guids)
        {
            if (guids.Count == 0)
            {
                return;
            ...
            bool ClearNewVariableNameDataProperty(
                int actionIndex, 
                SerializedProperty varProp, 
                CyanTriggerActionInstanceRenderData data,
                CyanTriggerActionVariableDefinition varDef)
            {
                ...
                var nameProperty = varProp.FindPropertyRelative(nameof(CyanTriggerActionVariableInstance.name));
                ...
                if (!guids.Contains(curGuid))
                {
                    return false;
                }

                var nameDataProperty = varProp.FindPropertyRelative(nameof(CyanTriggerActionVariableInstance.data));
                var nameData = CyanTriggerSerializableObject.ObjectFromSerializedProperty(nameDataProperty);
                if (nameData is string name)
                {
                    nameProperty.stringValue = name;
                    CyanTriggerSerializableObject.UpdateSerializedProperty(nameDataProperty, null);
            ...
            UpdateActionInputs(ClearNewVariableNameDataProperty, null);
        }
        ...
        public void DoLayoutTree()
        {
            ...
            Action delayedSelectedAction = null;
            CyanTriggerPropertyEditor.DrawButtonFooter(
                ...
                new Action[]
                {
                    ...
                    AddLocalVariable,
                    // These need to be called after OnGui for proper verification order.
                    () => delayedSelectedAction = DuplicateSelectedItems,
                    () => delayedSelectedAction = RemoveSelected
                },
                _isPlaying
            ...
            if (delayedSelectedAction != null)
            {
                delayedSelectedAction.Invoke();
            }
        ...
        private void DrawVariantSelector(Rect rect, RowGUIArgs args, CyanTriggerActionInstanceRenderData data)
        {
            ...
            if (data.ActionInfo.IsAction())
            {
                ...
                float docIconSize = 16;
                Rect docRect = new Rect(buttonRect.xMax - docIconSize, buttonRect.y + (buttonRect.height - docIconSize) * 0.5f, docIconSize, docIconSize);
                buttonRect.xMax -= docRect.width;
                
                Rect definitionRect = new Rect(buttonRect);

                string infoTooltip = "This action is a Custom Action. This means it was made through the combination of other actions. You can view the actions by clicking the \"View Actions\" button. You can view the definition describing the Custom Action by clicking the \"View Definition\" button. Click the ? button to view the wiki for Custom Actions"; 
                GUI.Label(labelRect, new GUIContent("Custom Action Info*", infoTooltip));
                
                CyanTriggerEditorUtils.DrawDocumentationButton(docRect, "Custom Actions", CyanTriggerDocumentationLinks.CustomAction);
                ...
                if (actionDefinition is CyanTriggerActionGroupDefinitionUdonAsset udonCustomAction)
                {
                    ...
                    string programTooltip = customProgram is CyanTriggerProgramAsset
                        ? "View the CyanTrigger that makes up this Custom Action."
                        : "View the Udon Program that makes up this Custom Action.";
                    if (GUI.Button(programRect, new GUIContent("View Actions", programTooltip)))
                    {
                        Selection.objects = new Object[] { customProgram };
                    }
                }
                
                if (GUI.Button(definitionRect, new GUIContent("View Definition", "View the Custom Action Definition.")))
                {
                    Selection.objects = new Object[] { actionDefinition };
                }
        ...
        public bool ContainsAnyInstanceCustomActions(List<int> actions = null)
        {
            if (actions == null)
            {
            ...
            foreach (var id in actions)
            {
                ...
                if (info.IsAction() && info.ActionGroup.isMultiInstance)
                {
                    return true;
                }
            }
        ...
        public void GetUsedVariables(
            List<int> actions,
            out List<CyanTriggerEditorVariableOption> inputVariables, 
            out List<CyanTriggerEditorVariableOption> outputVariables, 
            out List<CyanTriggerEditorVariableOption> missingVariables)
        {
            if (actions != null)
            {
                actions.Sort();
            ...
            HashSet<string> definedOutputGuids = new HashSet<string>();
            Dictionary<string, CyanTriggerEditorVariableOption> foundVariables =
                new Dictionary<string, CyanTriggerEditorVariableOption>();
            
            // Read before write = input
            // Write but not read = output
            
            HashSet<string> unreadOutput = new HashSet<string>();
            HashSet<string> everOutput = new HashSet<string>();
            HashSet<string> readBeforeWrite = new HashSet<string>();
            HashSet<string> allVariableIds = new HashSet<string>();

            // Special control actions will read variables after conditions:
            // If, Condition, ConditionBody, While, For, ...?
            Dictionary<int, List<string>> queueVariableReads = new Dictionary<int, List<string>>();
            
            foreach (var index in actions)
            {
                if (queueVariableReads.TryGetValue(index, out var guidsToRead))
                {
                    foreach (var guid in guidsToRead)
                    {
                        unreadOutput.Remove(guid);
                    }
                }
                
                var data = GetData(index + IdStartIndex);
                if (data == null)
                {
                    continue;
                }
                ...
                // Created variables
                var variables = info.GetCustomEditorVariableOptions(ItemElements[index]);
                if (variables != null && variables.Length > 0)
                {
                    foreach (var outputVariable in variables)
                    {
                        string guid = outputVariable.ID;
                        definedOutputGuids.Add(guid);
                        
                        if (foundVariables.ContainsKey(guid))
                        {
                            continue;
                        }
                        foundVariables.Add(guid, outputVariable);
                    }
                }

                List<string> allInputVariables = new List<string>();
                
                void ProcessInputProperty(SerializedProperty inputProperty, bool isOutput)
                {
                    ...
                    {
                        var varId = inputProperty.FindPropertyRelative(nameof(CyanTriggerActionVariableInstance.variableID));

                        string guid = varId.stringValue;
                        allInputVariables.Add(guid);

                        allVariableIds.Add(guid);
                        if (isOutput)
                        {
                            everOutput.Add(guid);
                            unreadOutput.Add(guid);
                        }
                        else
                        {
                            if (!everOutput.Contains(guid))
                            {
                                readBeforeWrite.Add(guid);
                            }
                            unreadOutput.Remove(guid);
                        }
                        ...
                        var allVariables = GetVariableOptions(typeof(System.Object), index);
                        foreach (var vari in allVariables)
                        {
                            if (vari.ID == guid)
                            {
                                foundVariables.Add(guid, vari);
                                return;
                            }
                        }
                    }
                ...
                SerializedProperty inputsProperty =
                    ItemElements[index].FindPropertyRelative(nameof(CyanTriggerActionInstance.inputs));
                SerializedProperty multiInputsProperty =
                    ItemElements[index].FindPropertyRelative(nameof(CyanTriggerActionInstance.multiInput));
                
                var varDefs = data.VariableDefinitions;
                for (int curVar = 0; curVar < varDefs.Length; ++curVar)
                {
                    var varDef = varDefs[curVar];
                    if (varDef == null)
                    {
                        continue;
                    }
                    
                    // Ignore inputs that do not support variables as input
                    if ((varDef.variableType & CyanTriggerActionVariableTypeDefinition.VariableInput) == 0)
                    {
                        continue;
                    }
                    ...
                    if (curVar == 0 && (varDef.variableType & CyanTriggerActionVariableTypeDefinition.AllowsMultiple) != 0)
                    {
                        // Go through multi input
                        int size = multiInputsProperty.arraySize;
                        for (int curMul = 0; curMul < size; ++curMul)
                        {
                            ProcessInputProperty(multiInputsProperty.GetArrayElementAtIndex(curVar), allowsOutput);
                        }
                        continue;
                    }
                    
                    ProcessInputProperty(inputsProperty.GetArrayElementAtIndex(curVar), allowsOutput);
                }
                
                if (Items[index].HasScope)
                {
                    int readIndex = Items[index].ScopeEndIndex;
                    queueVariableReads.Add(readIndex, allInputVariables);
                }
            }
            ...
            // unreadOutput and readBeforeWrite get marked.
            // Any other variable not in definedOutputGuids gets returned.

            inputVariables = new List<CyanTriggerEditorVariableOption>();
            outputVariables = new List<CyanTriggerEditorVariableOption>();
            missingVariables = new List<CyanTriggerEditorVariableOption>();

            foreach (var output in unreadOutput)
            {
                if (!foundVariables.TryGetValue(output, out var option))
                {
#if CYAN_TRIGGER_DEBUG
                    Debug.LogError($"Failed to find variable for guid {output}");
#endif
                    continue;
                }
                outputVariables.Add(option);
            }
            foreach (var input in readBeforeWrite)
            {
                // Already counted?
                if (unreadOutput.Contains(input))
                {
                    continue;
                }
                if (!foundVariables.TryGetValue(input, out var option))
                {
#if CYAN_TRIGGER_DEBUG
                    Debug.LogError($"Failed to find variable for guid {input}");
#endif
                    continue;
                }
                inputVariables.Add(option);
            }
            
            foreach (var otherVar in allVariableIds)
            {
                if (definedOutputGuids.Contains(otherVar) || readBeforeWrite.Contains(otherVar) || unreadOutput.Contains(otherVar))
                {
                    continue;
                }
                if (!foundVariables.TryGetValue(otherVar, out var option))
                {
#if CYAN_TRIGGER_DEBUG
                    Debug.LogError($"Failed to find variable for guid {otherVar}");
#endif
                    continue;
                }
                missingVariables.Add(option);
            }
        }
        ...
        protected override void GetRightClickMenuOptions(GenericMenu menu, Event currentEvent)
        {
            ...
            {
                GUIContent createCustomAssetGuiContent = new GUIContent("Create Custom Action from selected Actions");
                
                List<int> selection = new List<int>(GetSelection());
                selection.Sort();
                KeepOnlyParents(selection);

                // Reject if any Instance CustomActions since these are not proper variables that can be copied. Not like anyone uses them anyway
                bool anyInstanceCustomActions = ContainsAnyInstanceCustomActions(selection);
                bool validSelection = selection.Count > 0 && CanDuplicate(selection);
                
                if (anyInstanceCustomActions || !validSelection || _isSceneTrigger && !CyanTriggerProgramAssetExporter.CanExportAsset(_udonBehaviour))
                {
                    menu.AddDisabledItem(createCustomAssetGuiContent);
                }
                else
                {
                    menu.AddItem(createCustomAssetGuiContent, false, () =>
                    {
                        RequestCustomActionCreation(GetAllIdsIncludingChildren(selection, true));
                    });
                }
            }
            ...
            if (_mouseOverId != -1)
            {
                ...
                if (ItemCanExpand(data))
                {
                    ...
                    menu.AddItem(expandContent, false, () =>
                    {
                        ...
                        var item = GetItem(GetItemIndex(_mouseOverId));
                        item.displayName = GetElementDisplayName(data.Property, item.Index);
                    });
                }
                
                (GUIContent content, Action action) = CyanTriggerEditorUtils.GetDocumentationAction(data.ActionInfo);
                if (content != null)
                {
                    // Somewhat hacky way to remove the type name in the tooltip, but it works...
                    content.text = content.tooltip.Remove(content.tooltip.IndexOf(':'));
                    menu.AddItem(content, false, () => action?.Invoke());
                }
            }
        ...
        protected override void DoubleClickedItem(int id)
        {
            ...
            var item = GetItem(GetItemIndex(id));
            item.displayName = GetElementDisplayName(data.Property, item.Index);
...
#pragma warning restore CS0618
            ...
            for (int i = startIndex; i < Elements.arraySize; ++i)
            {
                ...
                {
                    var data = GetOrCreateExpandData(id);
                    data.IsExpanded = true;
                    data.NeedsRedraws = true;
                    _delayRefreshRowHeight = true;
                }
