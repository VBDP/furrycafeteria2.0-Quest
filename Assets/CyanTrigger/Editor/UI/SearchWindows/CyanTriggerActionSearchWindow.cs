...
namespace Cyan.CT.Editor
{
    internal class CyanTriggerActionSearchWindow : CyanTriggerSearchWindowProvider
    {
        ...
        private static void GetVrcActionDefinitions()
        {
            ...
            var definitionManager = CyanTriggerNodeDefinitionManager.Instance;
            foreach (var nodeDefinition in definitionManager.GetDefinitions())
            {
                ...
                // If not unique friendly name, add full namespace
                if (definitionManager.IsTypeFriendlyNameNotUnique(sortings[1]))
                {
                    secondaryKey = $"{sortings[1]} ({nodeDefinition.BaseType.FullName})";
                }
