...
namespace Cyan.CT.Editor
{
    internal class CyanTriggerVariableSearchWindow : CyanTriggerSearchWindowProvider
    {
        ...
        public bool allowRightClickAdd;
        ...
        {
            if (entry.userData == null && OnDefinitionSelected != null)
            {
        ...
        {
            var entries = allowCustomTypes ? CreateFullSearchTree() : CreateDefaultSearchTree();
            
            if (allowNull)
            {
                var allEntries = new List<SearchTreeEntry>();
                allEntries.AddRange(entries);
                allEntries.Insert(1, new SearchTreeEntry(new GUIContent("null")) {level = 1, userData = null});
