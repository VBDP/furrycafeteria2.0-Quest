    ...
    public class CyanTriggerActionGroupDefinitionUdonAsset : CyanTriggerActionGroupDefinition
    {
            ...
            string description = "")
        {
            ...
            SerializedProperty actionProperty = 
                AddNewEvent(eventListProperty, actionNamespace, eventName, baseEvent, eventName, "", description);
            ...
            string defaultNamespace = "")
        {
            ...
            customAction.defaultNamespace = string.IsNullOrWhiteSpace(defaultNamespace) ? programAsset.name : defaultNamespace;
            customAction.autoAddPriority = 0;
            customAction.isMultiInstance = false;
            
            customAction.Initialize();

            Dictionary<string, string> eventNameToComments = new Dictionary<string, string>();
            HashSet<string> seenEventNames = new HashSet<string>();
            foreach (var eventData in programAsset.GetCyanTriggerData().events)
            {
                var info = CyanTriggerActionInfoHolder.GetActionInfoHolder(eventData.eventInstance.actionType);
                var eventArgData = info.GetBaseEventArgData(eventData);

                if (seenEventNames.Contains(eventArgData.eventName))
                {
                    eventNameToComments.Remove(eventArgData.eventName);
                    continue;
                }
                seenEventNames.Add(eventArgData.eventName);

                string comment = eventData.eventInstance.comment.comment;
                if (!string.IsNullOrWhiteSpace(comment))
                {
                    eventNameToComments.Add(eventArgData.eventName, comment);
                }
            }
            ...
            {
                eventNameToComments.TryGetValue(selectedEvent.eventName, out string comment);
                customAction.AddEvent(eventsProp, selectedEvent, null, comment);
            }
        ...
        public static CyanTriggerActionGroupDefinitionUdonAsset CreateCustomActionForProgramAssetTest(
            CyanTriggerEditableProgramAsset programAsset,
            string defaultNamespace,
            List<string> inputVariableIds,
            List<string> outputVariableIds)
        {
            var customAction = CreateCustomActionForProgramAsset(programAsset, defaultNamespace);

            HashSet<string> inputVariables = new HashSet<string>(inputVariableIds);
            HashSet<string> outputVariables = new HashSet<string>(outputVariableIds);

            SerializedObject serializedObject = new SerializedObject(customAction);
            var eventsProp = serializedObject.FindProperty(nameof(exposedActions));
            var actionProperty = eventsProp.GetArrayElementAtIndex(eventsProp.arraySize - 1);
            SerializedProperty variablesProperty =
                actionProperty.FindPropertyRelative(nameof(CyanTriggerActionDefinition.variables));
            
            void AddVariable(string name, string displayName, Type type, bool isOutput)
            {
                AddNewVariable(
                    variablesProperty,
                    name,
            ...
            foreach (var variable in programAsset.GetCyanTriggerData().variables)
            {
                if (inputVariables.Contains(variable.variableID))
                {
                    AddVariable(variable.name, variable.name, variable.type.Type, false);
                }
            }
            
            foreach (var variable in programAsset.GetCyanTriggerData().variables)
            {
                if (outputVariables.Contains(variable.variableID))
                {
                    AddVariable(variable.name, variable.name, variable.type.Type, true);
                }
            }
