...
namespace Cyan.CT.Editor
{
    ...
    {
        private static readonly List<Type> ExternalTypesToProcess = new List<Type>();
        ...
        private readonly HashSet<string> _typesWithNonUniqueFriendlyNames = new HashSet<string>();
        ...
        public static void AddExternalCustomType(Type type)
        {
            if (_instance != null)
            {
                Debug.LogError($"Cannot add Custom Node type {type.FullName} as Custom Nodes have already been processed! Please use a static method with the [InitializeOnLoad] attribute to add Custom Nodes before processing.");
                return;
            }
            
            if (!type.IsSubclassOf(typeof(CyanTriggerCustomUdonNodeDefinition)))
            {
                Debug.LogError($"Cannot add Custom Node type {type.FullName} as it is not a subtype of {typeof(CyanTriggerCustomUdonNodeDefinition)}");
                return;
            }
            
            ExternalTypesToProcess.Add(type);
        }
...
#pragma warning restore CS0618
                ...
                // Checking if friendly names are unique or not.
                string friendlyName = CyanTriggerNameHelpers.GetTypeFriendlyName(type);
                if (!_typesWithNonUniqueFriendlyNames.Contains(friendlyName))
                {
                    if (_typeNamesToTypes.ContainsKey(friendlyName))
                    {
                        _typesWithNonUniqueFriendlyNames.Add(friendlyName);
                        _typeNamesToTypes.Remove(friendlyName);
                    }
                    else
                    {
                        _typeNamesToTypes.Add(friendlyName, type);
                    }
                }
            }
        ...
        private void AddCustomNodeDefinitions()
        {
            ...
            foreach (var type in ExternalTypesToProcess)
            {
                if (codeAssetGeneratorType.IsAssignableFrom(type) && 
                    !type.IsAbstract && 
                    type.GetConstructor(Type.EmptyTypes) != null) // ignores variable type
...
// #endif
        ...
        public bool IsTypeFriendlyNameNotUnique(string typeName)
        {
            return _typesWithNonUniqueFriendlyNames.Contains(typeName);
        }

        public IEnumerable<Assembly> GetUdonReferencedAssemblies()
        {
