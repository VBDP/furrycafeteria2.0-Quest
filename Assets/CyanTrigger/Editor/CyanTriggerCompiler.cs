    ...
    public class CyanTriggerCompiler
    {
        ...
        private void CheckShouldBeNetworked()
        {
            ...
            string[] networkActions =
            {
                ...
                CyanTriggerDefinitionResolver.GetMethodSignature(
                    typeof(IUdonEventReceiver).GetMethod(nameof(IUdonEventReceiver.SendCustomNetworkEvent), 0, BindingFlags.Public | BindingFlags.Instance, null, CallingConventions.Any, new []{typeof(NetworkEventTarget), typeof(string)}, null))
#else
                CyanTriggerDefinitionResolver.GetMethodSignature(
        ...
        private void AddUserDefinedVariables()
        {
            ...
            for (var curVar = 0; curVar < _cyanTriggerDataInstance.variables.Length; ++curVar)
            {
                ...
                var variableData = _data.AddUserDefinedVariable(
                    variable.name, 
                    variable.variableID, 
                    variable.type.Type, 
                    variable.sync,
                ...
                // Ensure default value for constant variables
                // This is required as some values cannot be properly serialized when public
                if (CyanTriggerUtil.IsTypeConstant(variable.type.Type))
                {
                    variableData.DefaultValue = variable.data.Obj;
                }
            }
            ...
            bool outputOnly)
        {
            ...
            // Keep default value for constant types as these will not be made exposed as publicly serializable.
            if (CyanTriggerUtil.IsTypeConstant(type))
            {
                varData.DefaultValue = input.data.Obj;
            }
