...
#pragma warning restore CS0612
            ...
            ref bool dirty)
        {
            ...
            foreach(string publicVariableSymbol in new List<string>(publicVariables.VariableSymbols))
            {
                ...
                // Prevent Constant variables as public
                if (CyanTriggerUtil.IsTypeConstant(type))
                {
                    //Debug.Log($"Removing Reference: {publicVariableSymbol}, type {type} as it is constant type");
                    publicVariables.RemoveVariable(publicVariableSymbol);
                    dirty = true;
                }
            }
            ...
            if (variables != null && userVariablesMap != null)
            {
                for (var index = 0; index < variables.Length; ++index)
                {
                    ...
                    // Skip constant variables as they cannot be set here.
                    if (CyanTriggerUtil.IsTypeConstant(variable.type.Type))
                    {
                        continue;
                    }
                    ...
                    value = VerifyVariableData(type, value,
                        () =>
                            $"Global variable \"{variable.name}\" contains invalid data for type {type.Name}. Data: {(messageValue != null? $"({messageValue.GetType().Name}) {messageValue}" : "null")}. Replacing with default value. Please check the CyanTrigger on object {VRC.Tools.GetGameObjectPath(udonObj)}");
            ...
            if (events != null && dataIndices != null)
            {
                ...
                {
                    // Ignore constant variable types.
                    if (CyanTriggerUtil.IsTypeConstant(publicVar.type.Type))
                    {
                        continue;
                    }
            ...
            out Type type)
        {
            ...
            data = VerifyVariableData(type, data, () => $"{message} contains invalid data for type {exportType.Name}. Data: {(messageData != null? $"({messageData.GetType().Name}) {messageData}" : "null")}. Replacing with default value. Please check the CyanTrigger on object {(gameObject == null ? "null" : VRC.Tools.GetGameObjectPath(gameObject))}");
        ...
        public static bool DeepEquals(object o1, object o2)
        {
            ...
            // More jank, but hopefully works for comparing two arbitrary objects that don't extend compare or equality
            if (t1.IsSerializable)
            {
                return CyanTriggerSerializableObject.EncodeObject(o1, out _) ==
                       CyanTriggerSerializableObject.EncodeObject(o2, out _);
            }
