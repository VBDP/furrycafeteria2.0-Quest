...
*/
        ...
        public static string GetProgramUniqueStringForCyanTrigger(CyanTriggerDataInstance instanceData)
        {
            ...
            // Variables
            {
                ...
                foreach (var variable in variables)
                {
                    ...
                    // Special handling for constant variable types,
                    // ex Type not supporting proper serialization and thus should be considered as a constant.
                    if (CyanTriggerUtil.IsTypeConstant(variable.type.Type))
                    {
                        triggerInfo.AppendLine(variable.data?.Obj?.ToString());
                    }
            ...
            bool reference = true)
        {
            ...
            {
                if (string.IsNullOrEmpty(variable.variableID) || variable.variableID == variable.name)
                {
            ...
            string data = "";
            if (!reference || CyanTriggerUtil.IsTypeConstant(def.type.Type))
            {
